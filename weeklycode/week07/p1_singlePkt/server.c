...
#include <sys/types.h> //API for manipulating sockets
#include <sys/socket.h> //API for manipulating sockets
#include <netinet/in.h> // headers for representing IPV4/IPV6 addresses
#include <dirent.h>
#include<sys/wait.h>

void checkError(int status) {
    if(status < 0) { //if status < 0, there is an error
...
int main(int argc, char* argv[]) {
    ...
    // ^^^ first argument for socket needs to be PF_NET or AF_NET^^^^
    // ^^^ second argument, SOCK_STREAM, says it will be looking for TCP requests^^^
    //stream socket = TCP, 
    //sid is a SOCKET FILE DESCRIPTOR
    ...
    int status = bind(sid, (struct sockaddr*)&addr, sizeof(addr)); //servers must always have bind()
