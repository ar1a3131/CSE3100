/*the following define statements correspond
 * to the values that the tag field should take, 
 * based on if val is holding a char, unsigned int or int*/
#define CH 0
#define UINT 1
#define INT 2
#define _GNU_SOURCE
...
int setUnion(Variant * tagged_union,int dType, void* value) {
  ...
  switch(dType) {
	...
	case(UINT):
    // TODO
    	tagged_union->val.uint = *(unsigned int*)value; break;
	case(INT):
    // TODO
...
void * getUnion(Variant * tagged_union) {
  switch(tagged_union->tag) {
	case(CH):
	return (void*)&tagged_union->val.ch;break;
	case(UINT):
    // TODO
    	return (void*)&tagged_union->val.uint;break;
	case(INT):
    // TODO
  	return (void*)&tagged_union->val.integer;break;
  }
  exit(-1);
}
...
void printUnion(Variant * tagged_union) {
  switch(tagged_union->tag) {
	...
	case(UINT):
    // TODO
	printf("%u", *(unsigned int*)getUnion(tagged_union)); break;
	case(INT):
    // TODO
...
int compareUnionAscending(const void * tagged_union1, const void * tagged_union2, void * fun_ptrs) {
  ...
  /*
    TODO: Fill this in. 
   */
  // Cast the fun_ptrs to the appropriate function pointer type based on the tag
    VarFun* functionPointers = (VarFun*)fun_ptrs;

    if (tU1->tag == tU2->tag) {
        int tag = tU1->tag;
        
        // Use the appropriate function pointer to compare based on the tag
        VarFun compareFunc = functionPointers[tag];

        return compareFunc(tU1, tU2);
    }

    exit(-1); // Handle error if tags are not the same
}
